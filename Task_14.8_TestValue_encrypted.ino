// Modified version of the transmitter code to capture an encrypted test value (42) for the oscilloscope waveform

#include <stdint.h>

const uint8_t SECRET_KEY = 0x54;  // 8-bit symmetric encryption key
...
void loop() {
  uint8_t cipher = encryptByte(TEST_VALUE);  // Encrypts test value (becomes 217)
  Serial.write(cipher);                      // Sends encrypted byte
  delay(100);
}
