...
// Generate a new 64-bit keystream block
static void refillKeystreamBlock() {
  ...
  blockCounter++; // Increments countr
}
...
void loop() {
  static uint8_t counter = 0; // Plaintext counter value (wraps automatically from 255 to 0)

  uint8_t pt = counter; // Plaintext byte
  uint8_t ks = nextKeystreamByte(); // CTR keystream byte
  uint8_t ct = pt ^ ks; // CTR encryption: C = P XOR KS

  Serial.write(ct); // Sends one encrypted byte (single UART frame)
  counter++; // Increments plaintext counter
